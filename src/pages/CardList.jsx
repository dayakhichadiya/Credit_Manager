...
const CardList = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center bg-gray-100 py-10">
      ...
      {cards.length === 0 ? (
        <p className="text-gray-600 text-lg">No saved cards found.</p>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {cards.map((card) => (
            <div
              key={card.id}
              className="relative bg-gradient-to-r from-blue-500 to-blue-700 text-white p-6 rounded-xl w-80 shadow-lg"
            >
              ...
              <div className="flex justify-between text-lg font-semibold mt-1">
                <span>{card.cardHolder || "Cardholder Name"}</span>
                <span>{card.expiry || "MM/YY"}</span>
              </div>
              <div className="text-xl font-bold tracking-widest my-4">
                {card.cardNumber
                  ? card.cardNumber.replace(/\d{12}(\d{4})/, "**** **** **** $1")
                  : "**** **** **** ****"}
              </div>
              <button
                className="absolute top-2 right-2 bg-red-600 hover:bg-red-700 text-white px-3 py-1 rounded-md text-sm shadow"
                onClick={() => handleDelete(card.id)}
              >
                Delete
              </button>
